...
const BookingSection: React.FC<BookingSectionProps> = ({
  ...
  // ✅ FIXED: Payment link creation with optional consultationTopic
  const handleCreatePaymentLink = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        consultationTopic: consultationFormData.consultationTopic || 'Astrology Consultation', // ✅ Default value
        createdBy: {
          userId: currentUser._id,
          userType: 'customer'
        },
        // ✅ Add all required fields from consultation form
      ...
      if (data.success) {
        // ✅ Store consultation log ID and start checking payment status
        setPaymentLinkStatus({
          consultationLogId: data.consultationLogId,
          status: 'pending'
        });
        
        await showShareModal(data.data.paymentLink, data.data.consultationLogId);
  ...
  // ✅ NEW: Function to check payment status periodically
  const startPaymentStatusCheck = (consultationLogId: string) => {
    ...
    paymentCheckInterval.current = setInterval(async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/customers/payment-status/${consultationLogId}`
        );
        const data = await response.json();
        ...
        if (data.success && data.data.paymentCompleted) {
          ...
          Swal.fire({
            ...
            text: 'The payment link has expired or failed. Please try again.',
            confirmButtonColor: '#980d0d'
          });
        }
      } catch (error) {
  ...
  const showShareModal = async (paymentLink: string, consultationLogId: string) => {
    ...
    Swal.fire({
      ...
      html: `
        <div class="text-left">
          <p class="text-sm text-gray-600 mb-3">This link has already been sent to the customer via SMS. You can also send it manually if needed.</p>
          ...
          <p class="text-xs text-gray-500 text-center mt-4">
            Thank you. If the user makes the payment, the booking will be confirmed automatically.
          </p>
        </div>
      `,
      showConfirmButton: false,
  ...
  return (
    <div className="lg:border rounded-xl lg:p-6 bg-white shadow-sm min-h-[600px]">
      ...
      <div className=" flex justify-between mb-6 pb-1 lg:pb-4 border-b border-gray-200">
        ...
        <div className='flex flex-col  items-end'>
         ...
         <img
                    ...
                    className="w-12 h-12 object-cover object-top hover:scale-105 transition-transform duration-300 cursor-pointer rounded-full"
                 
                  />
                  <h2 className='text-sm text-gray-500  space-y-2 hidden lg:inline-block'>{astrologerData.astrologerName}</h2>
                  </div>
                  </div>
      </div>

      {/* Payment Status Display */}
      {paymentLinkStatus && paymentLinkStatus.status === 'pending' && (
